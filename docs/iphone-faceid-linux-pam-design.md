...
`faceid-authd` は正規化済みの署名対象全体へ、ペアリング時に登録した key ID に対応する長期 Ed25519 verifier 秘密鍵で署名する。transport helper が運ぶ challenge envelope は次の要素で構成する。
...
初期版の暗号化チャネルは `faceid-transport` で終端する。したがって helper は平文のセッション内容を観測できるが、challenge の出所と完全性は verifier の Ed25519 署名、応答の完全性は Secure Enclave 署名によってエンドツーエンドに確保する。チャネル認証鍵と verifier 署名鍵は別の鍵であり、helper は verifier 秘密鍵を保持しない。相互認証チャネルは、盗聴防止、ペアリング済み端末以外からの要求抑止、DoS 耐性を高める defense-in-depth であり、helper を信頼境界へ含めるものではない。
...
auth required pam_iphone_faceid.so timeout=30
account required pam_permit.so
```

用途はモジュール引数ではなく `PAM_SERVICE=faceid-auth-test` から `faceid-auth-test.authenticate` として導出する。専用テストクライアントで動作確認した後、polkit と 1Password へ段階的に導入する。
