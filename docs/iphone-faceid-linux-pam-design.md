...
- 過去の署名応答の再利用
- 1Password 解錠用の署名を `sudo` や別 PAM サービスへ流用する攻撃
...
nonce、ユーザー名、PAM サービス名、操作、期限、端末識別子などは、root 所有の verifier が生成する。
...
`requested_action` は PAM サービス名だけに依存させず、可能な限り具体的にする。
...
1Password 専用利用を目的とする初期版では、許可する用途を `1password.unlock` に限定してもよい。
...
成功後、request ID を使用済みとして破棄し、再利用を拒否する。
...
```pam
auth sufficient pam_iphone_faceid.so timeout=30 action=1password.unlock
...
そのため初期版では、可能なら次のいずれかを採用する。

- 1Password の polkit action ID を verifier が確認する
- 1Password 用に明確な `requested_action` を生成する
- 許可対象 action を allowlist で制限する
- `pkexec` や管理操作には利用しない設定を既定とする

polkit から PAM へ action ID が直接伝わらない場合は、別の信頼できるコンテキスト伝達方法を設計する必要がある。これは実装前に検証すべき重要な未決事項である。
...
端末パスコードへのフォールバックを許可するかは製品ポリシーとして明示する。

初期版では、Face ID のみを要求するため、生体認証限定のポリシーを推奨する。
...
- verifier 停止時は fail closed とし、下段 PAM へフォールバックさせる
...
1. 1Password の polkit action を PAM/verifier 側で確実に識別できるか
2. 初期版を 1Password 専用に制限するか、汎用 PAM とするか
3. TLS 1.3 mutual authentication と Noise のどちらを採用するか
4. canonical encoding に CBOR、protobuf、JSON のどれを使うか
5. Face ID 失敗時に端末パスコードへのフォールバックを許可するか
6. Linux verifier の端末秘密鍵をどこに保存するか
7. transport helper をユーザーごとに起動するか、system service とするか
8. iPhone アプリのバックグラウンド動作制約をどう扱うか
9. 同時に複数 iPhone を呼び出すか、優先順位を付けるか
10. clock rollback 対策に monotonic clock をどこまで利用するか
11. PAM conversation に状態表示を出すか、デスクトップ通知に限定するか
12. 鍵登録と失効操作の管理 UX
13. 認証成功のキャッシュを許可するか
14. 1Password 以外の polkit 操作を既定で拒否できるか
...
- nonce だけでなく、ユーザー、用途、端末、期限を署名対象に含める
- Linux と iPhone を相互認証する
- 通信路を暗号化するが、通信路だけを信用しない
- 1Password 用署名を他の PAM 操作へ流用できないようにする
