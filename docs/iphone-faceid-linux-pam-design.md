...
確認コードは action ID の代替ではなく、polkit 要求との因果や用途を暗号学的に証明するものでもない。非特権プロセスに対する追加の認証要素としては扱わず、承認疲れと要求取り違えを緩和する UX 上の defense-in-depth と位置づける。PAM conversation でコードを表示できない場合、初期版は Face ID 認証を開始せず、下段の認証方式へフォールバックする。
...
- Linux 側に表示された確認コードをユーザーが正しく入力した
...
10. PAM conversation と iPhone 間の number matching を実装する
11. PAM モジュールを実装する
12. 専用 PAM サービスでテストする
13. polkit と統合する
14. 1Password で検証する
15. fuzzing、リプレイ、MITM、失効テストを行う
