...
Rust↔Swift で受理集合をバイト単位で一致させるための方式を次に固定する。

1. **未割当判定は自前の Unicode 16.0 Cn テーブル**（`General_Category=Cn`）で行う。UCD の
   `extracted/DerivedGeneralCategory.txt`（Unicode 16.0）から生成スクリプトで抽出し、生成元 URL・
   Unicode 版・SHA-256・スクリプトを固定して **repo に vendor**（ビルド時ネットワーク取得禁止）。
   Rust と Swift は**同一テーブル**を共有する。クレートの `is_public_assigned` 等は Private Use も
   除外し spec（Cn のみ拒否）より厳しくなるため使わない。
2. **NFC 判定**は各言語の標準機構を使う。Rust は**最新安定版 `unicode-normalization`**（NFC 変換を
   スカラー列比較して「入力が既に NFC か」を判定）、Swift は Foundation
   `precomposedStringWithCanonicalMapping`（**`String ==` は正準等価のため使わず** `unicodeScalars`/
   `utf8` の生列比較）。
3. **正規化安定性**により、上記1で「16.0 で割当済み」に gate した後は、より新しい Unicode 版の
   NFC 結果も 16.0 と一致する。よって Rust のクレート版は最新でよい（CLAUDE.md 方針を満たす）。
4. **Swift 側の Unicode 版保証は min OS pin（採用）**: Foundation が Unicode 16.0 以上であることを
   実測で確認できる**最低対応 OS を deployment target に設定**し、**Unicode 16.0 公式
   `NormalizationTest.txt` を CI で流して検証**する。
5. Rust/Swift の一致は、`spec/vectors/` の cross-language ベクタ（非NFC、16.0新規割当、16.0未割当、
   bidi/制御境界）と公式 NormalizationTest で担保する。
